...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  // Set by middleware.ts on every request (part of the CSP header itself)
  //, read back here so our one inline script (ThemeScript) can carry the
  // same nonce the CSP allowlisted for this response. Reading headers()
  // makes this layout dynamically rendered; a handful of low-traffic
  // static pages (design-system, offline, icons) lose build-time
  // pre-rendering as a result, every route that actually matters
  // (/buyer, /supplier, /admin, /api/*) was already dynamic (auth-gated)
  // before this, so the real-world cost is negligible next to closing off
